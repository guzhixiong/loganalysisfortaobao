...
public class IndexStay extends Configured implements Tool {
	...
	public static class M extends Mapper<LongWritable, Text, Text, Text> {
		...
		protected void map(LongWritable key, Text value, Context context)
			...
			if (!"1".equalsIgnoreCase(tmp[1])) {
				return;
			}

			context.write(new Text(tmp[0]), new Text(tmp[5] + " 1"));
		}
	...
	public int run(String[] args) throws Exception {
		...
		Job job = new Job(getConf(), date + "IndexStay");
		job.setJarByClass(IndexStay.class);
		...
		FileOutputFormat.setOutputPath(job, new Path(args[0] + "_index_stay"));
	...
	public static void main(String[] args) throws Exception {
		System.out.println("IndexStay Start...");
		System.exit(ToolRunner.run(new Configuration(), new IndexStay(), args));
	}
