...
public class IndexPvUv extends Configured implements Tool {
	...
	public static class M extends Mapper<LongWritable, Text, Text, Text> {
		...
		protected void map(LongWritable key, Text value, Context context)
			...
			if (!log.getPageType().equalsIgnoreCase("1")) {
				return;
			}
	...
	public int run(String[] args) throws Exception {
		...
		Job job = new Job(getConf(), date + "IndexPvUv");
		job.setJarByClass(IndexPvUv.class);
		...
		TextInputFormat.addInputPaths(job, args[0] + "_a");
		job.setMapOutputValueClass(Text.class);
		job.setOutputKeyClass(Text.class);
		job.setOutputValueClass(Text.class);
		FileOutputFormat.setOutputPath(job, new Path(args[0] + "_index_pv_uv"));
	...
	public static void main(String[] args) throws Exception {
		System.out.println("IndexPvUv Start...");
		System.exit(ToolRunner.run(new Configuration(), new IndexPvUv(), args));
	}
