...
public class KeywordPvUv extends Configured implements Tool {
	...
	public static class M extends Mapper<LongWritable, Text, Text, Text> {
		...
		protected void map(LongWritable key, Text value, Context context)
			...
			if (!log.getSearchType().equalsIgnoreCase("2")) {
				return;
			}
	...
	public int run(String[] args) throws Exception {
		...
		Job job = new Job(getConf(), date + "KeywordPvUv");
		job.setJarByClass(KeywordPvUv.class);
		...
		FileOutputFormat.setOutputPath(job,
				new Path(args[1] + "_keyword_pv_uv"));
	...
	public static void main(String[] args) throws Exception {
		System.out.println("KeywordPvUv Start...");
		System.exit(ToolRunner
				.run(new Configuration(), new KeywordPvUv(), args));
	}
