...
public class ShopAreaStay extends Configured implements Tool {
	...
	public static class M extends Mapper<LongWritable, Text, Text, Text> {
		...
		protected void map(LongWritable key, Text value, Context context)
			...
			context.write(new Text(shop + " " + province), 
					new Text(tmp[5] + " 1"));
		}
	...
	public int run(String[] args) throws Exception {
		...
		Job job = new Job(getConf(), date + "ShopAreaStay");
		job.setJarByClass(ShopAreaStay.class);
	...
	public static void main(String[] args) throws Exception {
		System.out.println("ShopAreaStay Start...");
		System.exit(ToolRunner.run(new Configuration(), new ShopAreaStay(),
				args));
	}
