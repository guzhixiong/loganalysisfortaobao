...
public class UserCategoryStay extends Configured implements Tool {
	...
	public static class M extends Mapper<LongWritable, Text, Text, Text> {
		...
		protected void map(LongWritable key, Text value, Context context)
			...
			if (!"4".equalsIgnoreCase(tmp[1])) {
				return;
			}

			context.write(new Text(tmp[0] + " " + tmp[3]), new Text(tmp[5]
					+ " 1"));
		}
	...
	public int run(String[] args) throws Exception {
		...
		Job job = new Job(getConf(), date + "UserCategoryStay");
		job.setJarByClass(UserCategoryStay.class);
	...
	public static void main(String[] args) throws Exception {
		System.out.println("UserCategoryStay Start...");
		System.exit(ToolRunner.run(new Configuration(), new UserCategoryStay(),
				args));
	}
