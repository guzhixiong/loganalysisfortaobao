...
public class PageFromPvUv extends Configured implements Tool {
	...
	public static class M extends Mapper<LongWritable, Text, Text, Text> {
		...
		protected void map(LongWritable key, Text value, Context context)
			...
			if (log.getPageType().equalsIgnoreCase("0")) {
				return;
			}
			...
			String[] tmp = new String[] { nick, pageType, pageID, from,
					landing, title, ip, "" + hour, "" + minute, "" + second };
	...
	public static class R extends Reducer<Text, Text, Text, Text> {
		...
		protected void reduce(Text key, Iterable<Text> values, Context context)
				throws IOException, InterruptedException {
			String[] tmp = Arrays.copyOfRange(key.toString().split(" "), 0, 6);
			Text k = new Text(StringUtils.join(tmp, " "));
	...
	public static class Sort extends TextComparator {
		...
		public int compare(Text o1, Text o2) {
			...
			if (cmp != 0)
				return cmp;
			return Integer.parseInt(a[9]) - Integer.parseInt(b[9]);
		}
	...
	public int run(String[] args) throws Exception {
		...
		Job job = new Job(getConf(), date + "PageFromPvUv");
		job.setJarByClass(PageFromPvUv.class);
	...
	public static void main(String[] args) throws Exception {
		System.out.println("PageFromPvUv Start...");
		System.exit(ToolRunner.run(new Configuration(), new PageFromPvUv(),
				args));
	}
