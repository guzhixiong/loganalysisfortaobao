...
public class Path1 extends Configured implements Tool {
	...
	private static final Log LOG = LogFactory.getLog(Path1.class);
	...
	public static class M extends Mapper<LongWritable, Text, Text, Text> {
		...
		protected void map(LongWritable key, Text value, Context context)
			...
			if (tmp.length < 10 || tmp[0].equals("-") || tmp[3].equals("-1")
					|| tmp[3].equals("30") || tmp[3].equals("31")) {
				return;
			}

			String[] tmp2 = { tmp[0], tmp[3], tmp[6], tmp[7], tmp[8], tmp[9] };
			context.write(new Text(StringUtils.join(tmp2, " ")), 
					new Text(tmp[1]));
		}
	...
	public static void main(String[] args) throws Exception {
		...
		System.exit(ToolRunner.run(new Configuration(true), new Path1(), args));
	}
